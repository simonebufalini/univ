...
def ex1(input_file,  output_file):
    ...
    start, end = (0, 0), (len(img)-1, len(img[0])-1)
    colors = [img[0][0]]
    rects = rec(img, start, end, colors)
    
    images.save([colors], output_file)
    return rects
    

def rec(img, start, end, colors, k=0):
    
    center = trova_centro(img, start, end)
    
    if center:
        colors.append(img[center[0]][center[1]])
        
        k += rec(img, center, end, colors)
        k += rec(img, (center[0], start[1]), (end[0], center[1]), colors)
        k += rec(img, (start[0], center[1]), (center[0], end[1]), colors)
        k += rec(img, start, center, colors)
...
def trova_centro(img, start, end):
    bg = img[0][0]
    sY = start[0] + 1
    sX = start[1] + 1
    eY = end[0] - 1
    ...
    rows = {y: img[y][sX] for y in range(sY, eY) if img[y][sX] == img[y][eX] and img[y][sX] != bg}
    
    if len(rows) >= 2:
        while True:
            row = rows.popitem()
            img_row = img[row[0]][sX : eX]
            
            if all([img_row[0] == img_row[i] for i in range(len(img_row))]):
                break
...
if __name__ == '__main__':
    ...
    ex1("puzzles\\rect02.in.png",  "out\\rect_02.png")
